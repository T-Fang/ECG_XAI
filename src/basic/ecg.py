...
class Ecg:
    ...
    def __init__(self, raw: NDArray[np.float32], metadata: pd.Series, preprocess: bool = False):
        """
    ...
    def delineate(self, method='dwt', check_P_inversion=True, check_T_inversion=True):
        ...
        self.delineations: list[dict[str, NDArray[np.int32]]] = get_all_delineations(self.cleaned, self.all_rpeaks, method=method)
    ...
    def get_cycles(self):
        ...
        self.all_cycles: list[list[CardiacCycle]] = get_all_cycles(self.all_rpeaks, self.delineations)
